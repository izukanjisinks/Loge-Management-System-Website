...
import { useLodges, roomImage } from '@/composables/useRooms'

const { lodges, loading, error, fetchLodges } = useLodges()

const search = ref('')

onMounted(() => fetchLodges({ page_size: 100 }))

const filtered = computed(() => {
  const q = search.value.toLowerCase()
  if (!q) return lodges.value
  return lodges.value.filter(l =>
    l.name?.toLowerCase().includes(q) ||
    l.address?.toLowerCase().includes(q)
  )
})

// Stable cover image per lodge using its index
const COVERS = [
  ...
  'https://images.unsplash.com/photo-1444201983204-c43cbd584d93?w=700&q=80',
  ...
  'https://images.unsplash.com/photo-1549294413-26f195200c16?w=700&q=80',
]

function coverImage(lodge, idx) {
  return lodge.logo_url || COVERS[idx % COVERS.length]
}
</script>
...
<template>
  <div class="max-w-[1280px] mx-auto px-5 md:px-16 py-10">
    ...
    <div v-if="error" class="py-16 text-center bg-(--color-error-container) rounded-2xl mb-6">
      ...
      <button class="font-sans text-sm font-semibold text-(--color-error) hover:underline" @click="fetchLodges({ page_size: 100 })">
        Retry
      </button>
    </div>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      >
        ...
        <div class="relative h-52 overflow-hidden">
          ...
          <!-- Logo badge -->
          <div v-if="lodge.logo_url" class="absolute top-3 left-3 w-10 h-10 rounded-full overflow-hidden bg-white ring-2 ring-white/60">
            <img :src="lodge.logo_url" :alt="lodge.name" class="w-full h-full object-cover" />
          </div>
