...
function validate() {
  ...
  if (mb.isCorporate) {
    ...
    if (isZS.value) {
      if (!mb.branchName)                e.branchName        = 'Required'
      if (!mb.departmentName)            e.departmentName    = 'Required'
      if (!mb.costCenter)                e.costCenter        = 'Required'
      if (!mb.glCode)                    e.glCode            = 'Required'
      if (!mb.bookedBy.jobTitle)         e.bookedByJobTitle  = 'Required'
      if (!mb.bookedBy.manNumber)        e.bookedByManNumber = 'Required'
  ...
  if (mb.participantMode === 'headcount') {
    ...
    if (mb.attendants.length > 1) {
      const emailsSeen = new Map()
      const phonesSeen = new Map()
      const idsSeen    = new Map()
      mb.attendants.forEach((att, i) => {
        if (att.email && !e[`att_${i}_email`]) {
          const k = att.email.toLowerCase().trim()
          if (emailsSeen.has(k)) {
        ...
        if (att.phone && !e[`att_${i}_phone`]) {
          const k = att.phone.replace(/\s+/g, '')
          if (phonesSeen.has(k)) {
        ...
        if (att.idNumber && !e[`att_${i}_id`]) {
          const k = att.idNumber.trim()
          if (idsSeen.has(k)) {
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
      ...
      <div class="lg:col-span-8 space-y-5">
        ...
        <template v-if="step === 1">
            ...
            <div class="px-6 py-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div class="flex flex-col gap-2">
                <label class="font-sans text-xs font-semibold tracking-widest uppercase text-(--color-on-surface-variant)">
                  {{ mb.costCenterType === 'internal_order' ? 'Internal Order No.' : 'Cost Centre' }} <span v-if="isZS" class="text-(--color-error)">*</span>
                </label>
              ...
              <div class="p-4 bg-(--color-surface-container) rounded-xl border border-(--color-outline-variant)">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div class="flex flex-col gap-1">
                    ...
                    <input v-model="mb.startDate" type="date" :min="today"
                      class="w-full bg-(--color-savannah-mist) rounded-lg px-3 py-2.5 font-sans text-sm text-(--color-on-surface) border-2 focus:outline-none transition-colors"
                  ...
                  <div class="flex flex-col gap-1">
                    ...
                    <input v-model="mb.endDate" type="date" :min="mb.startDate || today"
                      class="w-full bg-(--color-savannah-mist) rounded-lg px-3 py-2.5 font-sans text-sm text-(--color-on-surface) border-2 focus:outline-none transition-colors"
          ...
          <!-- ─── Approval Documents (corporate only) ─── -->
          <section v-if="mb.isCorporate" class="bg-(--color-surface-container-lowest) rounded-xl border overflow-hidden transition-colors"
